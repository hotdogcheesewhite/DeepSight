...
def create_train_json(scene_path, result_map):
    ...
    for i in range(1, nums-20):
        ...
        # import pdb; pdb.set_trace()
        prompt = get_prompt(command=command, his_trajs=his_trajs, speed_content=speed_content, bevtargetpoints = bevtargetpoints,FLAGE = FLAGE)     # 获取 prompt
        answer = get_answer(think_content=thick_content, bev_content=bev_content, future_trajs_pixel=future_trajs_pixel, future_trajs=future_trajs, decision = decision,summary = summary,FLAGE = FLAGE)  # 获取answer
        format_label = {
            "messages": [  
